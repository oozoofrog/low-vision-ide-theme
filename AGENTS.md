...
## 프로젝트 목적
- 저시력(Low Vision) 사용자에게 흔한 `대비 민감도 저하`, `눈부심(글레어)`, `시각 피로`를 고려한 IDE 테마를 설계/구현한다.
- 특히 녹내장 사용자 사례에서 출발한 요구사항을 포함하되, 더 넓은 저시력 상황에서도 유효한 원칙으로 일반화한다.
...
- 라이트/다크 쌍 제공: 새 테마는 `light`/`dark` 변형을 함께 만든다.
